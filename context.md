...
- **2026-09-03**: Homepage SEO keyword mapping, section alignment, and scalable review system.
  ...
  - Manually updated `public/sitemap.xml`: added `https://www.anaghkr.in/portfolio/` (priority: 0.9, weekly) and updated `https://www.anaghkr.in/about/` (priority: 0.8, weekly) and homepage lastmod timestamps. Verified in `dist/sitemap.xml`.
  - Removed unused package `papaparse` from `package.json`.
  - Implemented dynamic, high-performance scroll animations with side and top entrance effects:
    - Headers glide down smoothly from the **TOP** (`y: -24 -> 0`).
    - About section reveals with cinematic split effect: profile card glides from the **LEFT** (`x: -50 -> 0`), story & pillars glide from the **RIGHT** (`x: 50 -> 0`).
    - Feature cards, service cards, and selected works glide up from the **BOTTOM** with tight micro-wave delays (`index * 0.04s`, duration 0.38s, Apple-grade spring curve `[0.16, 1, 0.3, 1]`).
    - Added `rootMargin: "200px"` to `client:visible` in `index.astro` to ensure pre-scroll component hydration with zero delay. Verified build.
