...
export default function Navbar() {
    ...
    // Render mobile menu via portal so it's outside the nav's stacking context.
    // This prevents the nav's backdrop-blur / fixed positioning from clipping
    // the overlay — it always covers the full viewport on scroll.
    const mobileMenu = mounted && isMobileMenuOpen && createPortal(
        <div className="fixed inset-0 z-[200] md:hidden bg-[#050505] flex flex-col p-8 pt-24 animate-slide-in">
            <button
    ...
    return (
        <>
            ...
            {/* Mobile menu rendered at document.body via portal */}
            {mobileMenu}
        </>
    );
}